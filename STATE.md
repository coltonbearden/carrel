...
- 2026-09-09 (specs/22): `carrel index` covers source and config files as `FileType.CODE`,
  the walk honours `.gitignore` (shared with `pack` via `core/ignore.py`), and `DeskDB.rel()` /
  PDF manifest entries emit POSIX separators (D-010). Closes the "indexing source types is a
  planned follow-up" gap left by v0.2.0.
