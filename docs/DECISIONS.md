...
**A PDF pypdf refuses is bad input, not a carrel bug.** `handled` maps every `pypdf.errors.PyPdfError` — `LimitReachedError` included, a sibling of `PdfReadError` rather than a subclass — to exit 4 with the `--json` error object, and `main` sets pypdf's logger to `ERROR` unless `--debug` is given, because a 156-byte PDF with no `/Root` made pypdf log 50,000 warnings before its error.
