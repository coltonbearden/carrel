...
**A PDF pypdf refuses is bad input, not a carrel bug.** `core.output.pdf_refusal` classifies once, for `handled` and the MCP server alike: every `pypdf.errors.PyPdfError` — `LimitReachedError` included, a sibling of `PdfReadError` rather than a subclass — is exit 4; an encrypted file names `carrel edit pdf --decrypt`; pypdf's `DependencyError` (AES without `cryptography`) is exit 3 with the install command; and the two `PyPdfError`s pypdf raises for API misuse stay unexpected, because they mean a carrel bug. `main` silences pypdf's logger unless `--debug` is given, because a 156-byte PDF with no `/Root` made pypdf log 50,000 warnings before its exception.
