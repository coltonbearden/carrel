...
| Folder watch | `carrel watch` (`--recursive --existing --stable --poll --done-dir --error-dir --log --print-service`) | lib:watchdog (inotify, or `PollingObserver` with `--poll` for /mnt/c and shares), custom rule→action mapping via `core/actions.py`; settle-wait, filing away, JSON log, systemd/schtasks service printer | all | MVP (+ v0.4.0 flags) |
| Batch runner | `carrel batch` (`--jobs --manifest --resume --dry-run --fail-fast --json-lines`) | custom (`core/actions.py` shell actions per file, ThreadPool parallelism, JSON-lines manifest; walks like `index`) | all | v0.4.0 |
| Field extraction | `carrel fields` (`--profile --date-order --set --save`) | custom (`core/money.py` + `core/dates.py` + label heuristics over textextract; `core/patterns.py` for invoice/PO/IBAN/account; confidence per field) | all text-bearing | v0.4.0 |
| Inbox intake | `carrel intake INBOX --to DEST` (`--apply --watch --by ym\|period --fiscal-start --template --tag`) | custom pipeline over `fields` + `refs` + `rename` + `fsops` + `index` + `meta` + `tag`; scans OCRed into a searchable copy with the original kept under `_originals/` (D-014); dry-run default | all | v0.4.0 |
| Template rename | `carrel rename --template '{date}_{vendor}_{ref}{ext}'` | custom (fields/meta/refs placeholders, slugified, collision-safe; desk rows follow via `core/fsops.py`, which `organize --apply` now uses too) | all | v0.4.0 |
...
| Whole desk over MCP | `carrel mcp` | custom stdlib JSON-RPC 2.0 on stdio: 14 tools (search, pack, inspect, tag, note, index, convert, diff, redact, doctor, meta, fields, mail, refs) delegating to the command impl functions + resource templates `carrel://file/{path}`, `carrel://search/{query}` | all | v1 (3 tools) → v0.2.0 (10 tools + resources) → v0.4.0 (14) |
...
| Redaction | `carrel redact` | custom (pattern/regex redaction for txt/md/html/json/csv/xml with the built-in kinds from `core/patterns.py` — PII plus invoice/po/iban/routing/…; label-driven kinds keep the label); pdf: true redaction by rasterize→blackbox→rebuild (text layer destroyed by design; documented) | text types + pdf | v1 |
...
| Mail tools | `carrel mail attachments/split/threads/pst` | custom (attachments with sha256, mbox → dated `.eml` files, Message-ID/In-Reply-To/References threads); wrap:readpst (`pst-utils`) for Outlook `.pst`/`.ost` (`-e` one .eml per message, `-r` one mbox per folder) | eml mbox pst | v0.4.0 |
...
| Plugin surface generated from `--help` | 7 plugins (`carrel-inspect`, `carrel-convert`, `carrel-organize`, `carrel-documents`, `carrel-watch`, `carrel-agent`, `carrel-guard`) | custom; every command a slash command, usage blocks generated by `scripts/sync_plugins.py --check`; hooks `PreToolUse(Read)` → text conversion, `SessionStart` → capability summary, `PostToolUse(Write\|Edit)` → reindex | — | v0.2.0 |
| Source-tree indexing | `carrel index` (`--no-source`, `--no-gitignore`) | custom (`FileType.CODE` from `SOURCE_EXTENSIONS`; `.gitignore` walk shared with `pack` via `core/ignore.py`) | source + config types | post-v0.2.0 |
...
- **`pack --query` ranks indexed files**, and since spec 22 `carrel index` covers plain-text
  source and config files (`.py`, `.rs`, `.toml`, `.yaml`, `Makefile`, …) as type `code`, so
  query-driven packing works on source trees as well as document trees. `--no-source` indexes
  documents only. What is still never walked: hidden entries (`.git`, `.carrel`, dotfiles),
  `.gitignore`d paths (`--no-gitignore` opts out), and binary types with no extractable text.
