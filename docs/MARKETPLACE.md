...
| **carrel-guard** | — | `PreToolUse` Read guard + `SessionStart` capabilities hook (below) | Read docx/odt/epub/rtf, xlsx and eml/mbox as plain text — the formats `Read` cannot open — plus PDFs as cheap text by default; images stay pictures unless `CARREL_GUARD_OCR_IMAGES=1`. And start every session knowing carrel's capabilities |
...
Claude Code's `Read` tool understands text, images and PDFs (PDFs in `pages` ranges past ten
pages) — and nothing else. `carrel-guard` registers a `PreToolUse` hook on `Read` that
converts the formats it cannot open — `.docx .odt .epub .rtf .xlsx .eml .mbox .mbx` — to text
with carrel into a cache directory, and rewrites the Read to the text file. PDFs are converted
too, because page images cost far more tokens than the text (`CARREL_GUARD_PDF_TEXT=0` keeps
the visual Read when layout matters). Images are **left alone**: `Read` already shows Claude
the picture, and OCR would replace a chart or a screenshot with a worse transcription
(`CARREL_GUARD_OCR_IMAGES=1` opts in). Claude reads the whole document as text in one call;
the source is never modified, and the note says where it still is.
...
> carrel 0.5.0 is on PATH: 32 of 33 commands ok, 1 degraded, 0 unavailable. Run `carrel doctor --json` for the full table and `carrel <cmd> --help` before composing flags. Most useful missing binaries: readpst (sudo apt install pst-utils), edge-tts (pipx install edge-tts), piper (pipx install piper-tts).
...
The server (pure stdlib JSON-RPC over stdio, protocol `2025-06-18`) exposes fourteen tools and
...
| `carrel_fields` | Extract vendor, invoice number, dates, subtotal/tax/total, currency and IBAN from a document with a confidence per field; `save` writes them as desk fields |
| `carrel_mail` | Save an email's attachments into a directory, or group eml/mbox messages into threads |
