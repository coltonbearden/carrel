...
Usage: carrel pack [OPTIONS] PATHS...
  ...
  tree, files}). Token estimates are ceil(chars / 3.6), labeled tokens_est.
  ...
  containing `/` match relative to their .gitignore's directory. Negation (`!pattern`) is NOT
  supported — such lines are ignored. `.git` and `.carrel` are always skipped. Binaries outside the
  supported set are listed in the tree as [skipped: binary] with their size, never inlined; images
  are only read (OCR) with --ocr.
...
Options:
  ...
  --max-bytes N           Stop adding file contents once N total bytes are packed; omissions are
                          noted in the header.
  --max-file-bytes N      Skip any single file larger than N bytes.
  --chunk TOKENS          Split into OUT.part1..N, each at most TOKENS estimated tokens (requires
                          -o). Files are never split mid-file unless one alone exceeds the budget;
                          then it is split on line boundaries with (continued) markers.
