...
5. **Propose the filing.** `carrel --json intake INBOX --to DEST` (dry-run) or `carrel --json rename PATH... --template '...'` (dry-run). Present the plan as a table: source → destination, plus every skip and its reason. **Only after the user agrees** re-run with `--apply`. Never pass `--apply` on the first call, never invent a `--force`.
