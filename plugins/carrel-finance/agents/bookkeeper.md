...
**If `--apply` exits 2 saying it would rewrite a git work tree, stop and tell the user.** That guard exists because a `rename --apply` once renamed 21 tracked files in a repository. `--force` overrides it and you never pass it on your own initiative — report the repository root the message names and let the user decide. Filing a document directory that happens to sit inside a repo is a real case, so the answer is usually "point it somewhere else", not "force it".
