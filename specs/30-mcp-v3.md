...
| `carrel_batch` | `batch` | `dry_run` default; no `--run` shell without an explicit opt-in |
...
1. **Mutating tools are dry-run by default.** `rename`, `intake`, `organize` and
   `batch` take `apply: false` unless the caller says otherwise, and the result
   carries the same plan the CLI prints. An agent that forgets the flag gets a
   plan, not a filesystem change.
...
with: `tools/list` returns 26; every name in `TOOLS` dispatches; each mutating
...
`carrel://search/{query}`), and any notion of confining a tool call to a desk
root — the server is local stdio with the caller's own privileges, and the
per-call `root` argument already points anywhere (documented in TEST_REPORT).
